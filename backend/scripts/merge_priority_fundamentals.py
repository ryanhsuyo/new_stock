#!/usr/bin/env python3
"""預覽或合併 Buffett priority fundamentals CSV。"""
...
def _print_result(result: dict) -> None:
    ...
    preview = result.get("buffett_preview") or []
    if preview:
        print("  Buffett 預覽:")
        for item in preview[:10]:
            code = item.get("code")
            name = item.get("name") or code
            score = item.get("buffett_score")
            signal = item.get("buffett_signal") or ""
...
def run_merge(args: argparse.Namespace) -> int:
    ...
    if args.apply and result.get("signals_refresh_required"):
        ...
        print("  驗收          : 重跑 signals 後，doctor 的 Buffett 覆蓋率應上升。")
    return 0
