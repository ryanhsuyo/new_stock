...
def test_daily_check_signal_alert_payload_includes_review_checklist():
    import daily_check
    ...
    alerts = {
        ...
        "alerts": [
            ...
            },
            {
                ...
                "title": "持股/候選轉風險",
                "action_label": "先復盤風險",
                "review_focus": ["previous_key_price", "current_signal", "current_action"],
            },
        ],
    }
    ...
    payload = summary["top_actions"][0]["action_payload"]
    assert payload["review_focus_counts"] == {
        "current_action": 2,
        "current_signal": 1,
        "previous_action": 1,
        "previous_key_price": 1,
    }
    assert payload["review_checklist"] == [
        "2330 台積電：先復盤風險；檢查 previous_key_price / current_signal / current_action。",
        "2408 南亞科：比對動作；檢查 previous_action / current_action。",
    ]
    assert payload["review_checklist_copy_text"] == (
        "訊號快照復盤清單\n"
        "1. 2330 台積電：先復盤風險；檢查 previous_key_price / current_signal / current_action。\n"
        "2. 2408 南亞科：比對動作；檢查 previous_action / current_action。"
    )
    assert payload["preview_items"] == [
        "2330 台積電：持股/候選轉風險｜先復盤風險",
        "2408 南亞科：前次快照以來動作變更｜比對動作",
