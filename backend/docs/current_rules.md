...
- `manual_market_note` 的 Daily Check API payload 必須提供 `required_fields`、`writing_checklist` 與安全的空白 `example_payload`；範例只能填資料日與 `source=manual`，不得由系統捏造 headline、risk_level 或盤勢內容。
- `GET /api/system/daily-check` 讀取舊的 `daily_check.json` 時，後端必須補上 `snapshot_is_stale`、`snapshot_stale_reason`、`snapshot_refresh_command`、`snapshot_refresh_copy_command` 與 `snapshot_refresh_expected_outputs`；交易日以當日快照為準，週末 / 非交易日以最近一個已完成交易日為準，避免週末凌晨把最新交易日摘要誤判為需刷新。
...
- 測試不得寫入正式 `backend/out/*` 訊號輸出；任何直接呼叫 `run_daily_signals()` 的測試都必須使用 `tmp_out` 或等效 monkeypatch 隔離輸出，避免測試日期污染 Dashboard / Daily Check / signal_alerts。
...
- `today_scan.json.usage_status` 可在不重算候選分桶的情況下由後端 service 依最新 `daily_check.json` 重新整理；例如 signal-alert acknowledgement 解除 Daily Check blocker 後，Today Scan usage status 必須同步改為可用或下一個後端判定狀態。
...
- Quality Momentum Lite 的直接 apply path 決策為 PE-only：除非先新增明確命名的 report-derived reference 欄位與測試，否則不得把官方暫存報告中的當期營益率、負債 / 權益輸入、月營收 YoY 或 EPS 寫入既有正式欄位。
...
- `GET /api/system/data-status` 與 Update Workflow checks 必須提供後端產生的 `manual_update_action`，包含短版 command、完整 `copy_command` 與 `expected_outputs`；此為手動 fallback 導引，不代表系統已安裝或啟用排程。
