...
- Update Workflow 的 `next_action.expected_outputs` 必須列出跑完指令後應檢查的主要產物；`daily_update.py` 流程至少包含 `ohlcv.csv`、`update_status.json`、`summary.json`、`universe_report.csv`、`daily_brief.json` 與 `daily_check.json`。
...
- `core`：核心技術策略，負責短線/波段正式買點與風控。
- `old_wang`：老王短波段 / 大盤籌碼輪動，負責強勢族群、短均線、爆大量低點與不追高判斷。
- `buffett`：巴菲特長期品質價值，負責基本面品質、安全、估值與成長觀察；不參與短線買賣訊號。
- Buffett 基本面補資料流程必須由後端 `fundamentals-status.workflow_summary` 提供 PM 階段、主要下一步、checklist 與焦點股票；前端只呈現此摘要，不自行重建流程規則。

不要再新增第四種推薦方案；若需要混合判斷，應在單檔說明裡呈現「共振」，不要另開 `combined` 推薦桶。

策略共振欄位：

- `strategy_alignment`：`strong_alignment` / `single_strategy` / `conflict` / `no_alignment`
- `aligned_strategies`：列出同向支持的策略，例如 `core`、`old_wang`、`buffett`
- `strategy_conflict_notes`：列出衝突原因；核心技術若已出場或失效，老王 / 巴菲特不得覆蓋風控結論。
...
- 預覽合併必須回報可合併檔數、部分填寫檔數、空白檔數、格式警告與 `buffett_preview` 試算。
- Dashboard 顯示 `buffett_preview` 時，除總分外必須顯示品質 / 安全 / 估值 / 成長四組分數，避免只看總分而不知道卡在哪一類。
- 正式合併後必須重新產生訊號，Buffett 候選才會進入 `summary.json` / `universe_report.csv`。
- Dashboard 在正式合併成功且 `signals_refresh_required=true` 時，必須直接提供「重新產生訊號」入口，避免使用者忘記刷新 Buffett 候選。
- CLI 正式合併成功後，必須提示重跑 `python3 scripts/run_signals.py`，並更新 `fundamentals_report.json` 供 Dashboard / doctor 讀取。
- Workflow 若偵測到 priority CSV 已可合併，下一步應提示「合併巴菲特基本面補資料」而不是只顯示一般缺資料提醒。
...
- `buffett_quality_score`：品質
- `buffett_safety_score`：財務安全
- `buffett_value_score`：估值
- `buffett_growth_score`：成長

成立條件：

- `buffett_flag=true` 代表長期品質價值觀察成立。
- 此方案不產生短線買賣點，也不覆蓋 `core` 或 `old_wang`。
- 缺少 `fundamentals.json` 時，必須回傳 `buffett_data_ok=false` 與缺資料原因。
- 若單檔已有部分基本面資料，四組評分（品質 / 安全 / 估值 / 成長）至少 3 組可用即可先評分。
- 部分資料評分必須輸出 `buffett_data_completeness_pct`、`buffett_missing_fields`、`buffett_scored_groups`，不可假裝資料完整。
- 少於 3 組可用時維持 `buffett_data_ok=false`，並在缺資料原因說明目前可用組數與缺欄位。
...
- `python3 scripts/doctor.py` 是每日健康檢查入口；必須檢查 Python 套件、輸出檔日期同步、候選股資料完整率、Buffett 覆蓋、Buffett priority CSV 狀態 / 可合併檔數 / 第一筆錯誤、候選股復盤缺口、候選股復盤 Markdown 草稿是否已產生與交易紀錄格式，並用 exit code `0/1/2` 表示 OK / WARN / BLOCK。
...
- `doctor.py` 若偵測 `fundamentals_report.priority_fill_readiness.status=ready_to_merge`，下一步應提示合併 priority CSV 並重算 signals；若 `status=invalid`，必須升級為 BLOCK 並顯示第一筆錯誤 / 警告，避免使用者只看到「Buffett 0/N」卻不知道卡在哪裡。
...
- `workflow-status.readiness_review` 是每日資料閉環總表，固定彙整資料日、交易報表、人工盤後筆記、巴菲特基本面與持股決策紀錄；若有 blocked，`top_blocker_key` 必須指向第一個應處理的區塊。
- `workflow-status` 應追蹤候選股報表可行動清單（可小試、等回測、減碼、出場）的決策日誌覆蓋率；僅計入 `source='universe_report'` 的同資料日紀錄，避免手動筆記誤判為報表復盤完成。
- Dashboard 顯示巴菲特/基本面狀態時，應優先使用 `GET /api/system/fundamentals-status` 的 `next_fill_targets`、`field_missing_counts` 與覆蓋率；只能呈現缺資料摘要，不得用前端補算長期價值訊號。
...
- `priority_reduce` 優先級高於續抱與共振；若核心技術已轉風險，不得因老王 / 巴菲特成立而改列續抱。
...
- `bucket` 必須與每日汰弱留強分類同邏輯；若是長期 Buffett 觀察但沒有短線買點，可列 `long_watch`。
