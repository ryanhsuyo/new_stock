...
**護欄接 production + 訊號前推驗收（2026-08-02）**：組合護欄從 evaluation-only 改為實際擋下台股每日進場候選，台股報告拆成三份 per-profile artifact；台股／美股報告在今日結論前印出「策略當前證據狀態」；美股開始每日保存 signal snapshot。

**這一輪推翻了先前「不得接 production」的結論，前提必須一起讀**（詳見下方 Completed 與 `validation.md`）：護欄回放從未通過，接上去的理由不是它會賺錢，而是（1）使用者同時決定台股停止新單，（2）護欄只會減少候選、不會增加候選。**任一前提失效——尤其恢復下單——就必須重新檢視。** 回推 54 筆顯示護欄留下的比擋下的更差（−8.33% vs −4.75%）：它限制曝險，不改善選股。**非推薦、非買賣建議、不下單。**
...
- **護欄接上 production（2026-08-02，明知未通過驗收仍接上，使用者決定）**：`guardrails-to-production`。先前結論是「三模式仍虧損、不通過、不得接 production」，該結論**至今仍成立**——護欄回放 2026-07-01～07-29 為 −10.06%，06-01 起 −20.0%，只有 05-04 起為 +10.0%（起始日決定正負號）。仍接上的理由與前提：
  - 護欄的目的改為**限制曝險**，不宣稱提升報酬。回推 54 筆：護欄留下 33 筆平均 −8.33%／勝率 9.1%，擋下 21 筆平均 −4.75%／勝率 14.3%——**護欄留下的比擋下的更差**，它取 `daily_priority`／`score` 前 N 名，而該排序在此樣本上與後續報酬反向。
  - 前提一：使用者同一輪決定**台股停止新單**，所以未通過的護欄目前不會決定任何真錢。
  - 前提二：護欄只會**減少**候選，不會增加；接錯的最壞情況是少看幾檔，不是多買幾檔。
  - **失效條件：恢復台股下單時，必須重新檢視是否該關掉 `GUARDRAILS_ENABLED`。** 這個開關存在就是為了這件事。

- **訊號前推驗收與證據狀態（2026-08-02）**：`signal_forward_validation_service` 以快照 `generated_at` 次一交易日開盤為進場基準（用 `as_of` 會拿到當時尚未產出的訊號，實測偏誤 −7.17% vs −6.94%），出場取訊號當日記錄的失效價。台股近 30 天 19 個快照日、37 個 BUY 訊號：**平均 −6.35%、勝率 10.8%、50/54 觸停損**；同期市場中位 −14.18%（80.5% 下跌）、0050 −6.33%。此數字現在印在每日報告的今日結論**之前**。發現並修好一個 bug：報告的進場篩選從未使用 `old_wang_market_filter`，「風險模式」只是一行提示文字，一列都沒擋掉。

- **美股 signal snapshot（2026-08-02）**：`us-signal-snapshots`。美股沒有台股那種每日訊號流程，分桶只存在於產生報告的瞬間，所以美股證據狀態一直是樣本 0。現在每日寫出 `out/us_signal_snapshots/`；只有「可紙上追蹤」桶計分（觀望桶沒有發生過進場），出場只用型態失效價（量幅目標當停利會把贏家封頂、輸家留到停損）。首份快照 2026-07-31：15 檔全保存、可追蹤 0 檔。W 底回放顯示訊號頻率約每年 59 筆（每 4 個交易日 1 筆），故樣本可望在數月內累積；**不補建歷史快照**（重算會帶進生存者偏差）。

- 風險恢復確認與停損重進診斷（2026-07-29，evaluation-only）：驗收報告列出停損後同標的再次進場的精確交易日間隔與停損／重進日風險。近期合併／老王各 3 次（2、3、10 個交易日），均由 defensive 恢復 normal；穩健動能 0 次。新增「防守後首個 normal 日先使用 watch 容量」並以四窗口有／無規則對照：12 個策略×窗口中 9 組改善或持平；近期三模式改善至 −10.06%／−10.66%／−10.90%，但 6–7 月合併與穩健動能各小幅惡化約 0.1%，長窗口合併 MDD 增加 0.82%。保留於 evaluation，不改 production。**（此「不改 production」已於 2026-08-02 被上方 `guardrails-to-production` 取代；虧損結論本身未被推翻。）**

- 停損當日重進修正（2026-07-29，evaluation-only）：稽核發現 2408 在計畫停損當日仍以同日收盤訊號排入隔日重買；回放現在使該已失效訊號失效並留下 `same_day_stop_reentry`，沒有加入任意 3／5 日冷卻。相同 2026-07-01～07-29 窗口，合併由 −13.35%／MDD 13.35% 改善為 −11.31%／11.31%，老王由 −13.67%／13.98% 改善為 −11.23%／11.54%，穩健動能由 −14.91%／15.27% 改善為 −13.09%／13.45%。機制修正有效但三模式仍虧損，結論維持不通過。

- 官方事件摘要與近期壓力驗收（2026-07-29）：盤前風險 API／Dashboard 新增未來 14 天人工查證的 Fed／BLS 官方事件、原始來源與查證新鮮度；事件不納入分數，也不宣稱可預測突發新聞。策略驗收回放現在實際執行訊號日 `stop_price`：盤中觸價依停損價、跳空跌破依開盤價，兩者均套既有賣出滑價；最新報告依檔案修改時間載入，不再被檔名字典序誤導。2026-07-01～07-29 壓力窗口雖較修正前改善，但三模式仍皆虧損（合併 −13.35%／MDD 13.35%、老王 −13.67%／13.98%、穩健動能 −14.91%／15.27%），且各有 10～14 次計畫停損，結論維持 **不通過**。**（「不得接 production」已於 2026-08-02 被上方 `guardrails-to-production` 取代——護欄在未通過的狀態下接上，前提是台股停止新單；「不通過」本身仍然成立。）**

- 日報與風險語意修正（2026-07-29）：Daily Check 寫入後會同步刷新 Today Scan；Today Scan 與台股日報在逐股資料日未全部對齊 summary `as_of` 時，明確阻擋交易輸出及候選／降風險清單。台股報告排程由 15:00 改為 15:40，等 15:30 日常更新後執行；launchd 已載入 15:40 並補跑成功。大盤濾網為風險模式時，Today Scan 改顯示「只供觀察與風險處理」，老王觀察桶不再被誤讀成進場清單；台股日報把 `block`／`exit_warning` 翻譯為「風險模式」／「出場警示」。盤前風險零分狀態由「正常」改為「未觸發額外防守」，明示不代表今日不會大跌，且不可因此放寬停損。交易頁把自動 OHLCV 檢查稱為「價格範圍正常／待確認」，不冒充人工核對；Dashboard 驗收摘要顯示回放區間、產生日期，歷史報告不得當作今日策略狀態。實測先在 68/76 檔對齊時成功封鎖，15:47 補跑後為 76/76；目前因大盤濾網封鎖而處於 observation-only。完整 backend **999 passed**、frontend build、Monitor targeted **4 passed**。

- 日常報告完整性與可修正流程（2026-07-24）：台股拆為精簡的每日行動報告與獨立歷史交易稽核；實際 15 筆交易中 9 筆成交價落在同日 OHLCV 範圍外，因此績效明示「暫估」，未自動改動交易。交易紀錄頁可逐筆檢視狀態並人工修正，後端會先備份、重算金額與檢查持股。美股已移除主要決策清單與 W 底詳表的重複已突破標的，並以固定 `as_of` 證明加入未來極端行情不改變歷史日輸出。Monitor 另有手動「台股交易稽核」卡片，不與日常報告或 Discord 午報混用。完整 backend **987 passed**、frontend build、Monitor **49 passed**。

- 美股每日可執行決策表（2026-07-24）：New Stock 報告不再把所有觀察標的一律當作今天可做。`us_trend_follow` 只有 bullish gate + candidate 才標示次一交易日紙上追蹤；mixed 等待大盤同步；保護線呈現 evaluation-only trend-protect（連 2 日跌破 MA20 或跌破 MA60）。`us_wbottom_target` 只有 breakout_today + gate active 才可紙上追蹤，breakout_in_progress 一律標示原始 D+1 已過、不追價。最新實際結果：trend gate bearish、W 底 gate inactive，今日新紙上追蹤 0 檔，4 檔僅追蹤既有型態。

- 美股報告推薦觀察清單（2026-07-23）：New Stock 策略報告會先列 `us_trend_follow` 候選，再補未重複的 `us_wbottom_target` 已突破標的；forming／invalidated 不納入。每檔顯示優先序、策略來源、守線／觸發、失效條件、觀察目標與理由。此清單是研究排序，不是正式推薦桶、買進指令或實際績效。

- Monitor Dashboard 分市場報告（2026-07-24；台股時間於 2026-07-29 修正）：日常卡片已拆成 `New Stock 美股報告`（10:00）與 `New Stock 台股報告`（15:40），各自使用獨立 launchd label、artifact、log 與重跑命令；執行時先刷新對應市場資料再產報告。合併報告只保留手動 `--market all` 相容用途，不再顯示於 Monitor，也不與 Discord 老王午報混用。

- Dashboard 策略驗收摘要（2026-07-23）：首頁讀取既有 `GET /api/system/strategy-validation`，顯示目前保存報告數、最新區間與合併／老王／穩健動能的報酬、MDD、買賣數及風控略過數；可直接跳到完整策略驗收頁。報告 API 失敗只降級此卡片，不阻塞 Dashboard。實際確認保存 4 份；完整 backend **969 passed**、frontend build 成功，桌面跳轉與 375px 無橫向溢出、console 0 errors。
