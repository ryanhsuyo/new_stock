...
- 風險恢復確認與停損重進診斷（2026-07-29，evaluation-only）：驗收報告列出停損後同標的再次進場的精確交易日間隔與停損／重進日風險。近期合併／老王各 3 次（2、3、10 個交易日），均由 defensive 恢復 normal；穩健動能 0 次。新增「防守後首個 normal 日先使用 watch 容量」並以四窗口有／無規則對照：12 個策略×窗口中 9 組改善或持平；近期三模式改善至 −10.06%／−10.66%／−10.90%，但 6–7 月合併與穩健動能各小幅惡化約 0.1%，長窗口合併 MDD 增加 0.82%。保留於 evaluation，不改 production。
